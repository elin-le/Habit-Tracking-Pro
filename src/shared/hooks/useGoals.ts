...
export type StatusFilter = 'ALL' | 'IN_PROGRESS' | 'COMPLETED' | 'NOT_STARTED' | 'NEAR_COMPLETION';
...
export const useGoals = () => {
    ...
    const filteredGoals = useMemo(() => {
        return goals.filter((g) => {
            const matchStatus = statusFilter === 'ALL' || (statusFilter === 'NEAR_COMPLETION' ? g.progress.progressPercent >= 80 && g.progress.status !== 'COMPLETED' : g.progress.status === statusFilter);
